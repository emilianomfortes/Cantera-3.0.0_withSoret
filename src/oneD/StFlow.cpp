...
void StFlow::setTransport(shared_ptr<Transport> trans)
{
    ...
    // Mixutre averaged transport model
    m_do_mixavg = (m_trans->transportModel() == "mixture-averaged");\
    if (m_do_mixavg) {
        m_dthermal.resize(m_nsp, m_points, 0.0);
    }
    ...
    if (m_do_multicomponent) {
        ...
        for (size_t j = j0; j < j1; j++) {
            ...
            for (size_t k = 0; k < m_nsp; k++) {
                ...
                // Correction velocity for Soret effect
                //if (m_do_soret) {
                //    double gradlogT = 2.0 * (T(x,j+1) - T(x,j)) /
                //                    ((T(x,j+1) + T(x,j)) * (z(j+1) - z(j)));
                //    m_flux(k,j) += m_dthermal(k,j)*gradlogT*Y(x,k,j);
                //}
